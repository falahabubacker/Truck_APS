...
class ParkingLotEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.stage_1_distance_threshold = 0.0714  # meters
    ...
    def reset(self, seed=None, options=None):
        ...
        spawn_point = random.randrange(1,5,1)
        spawn_randomness = 1.5
        ...
        spawn_transform.location.y += random.uniform(-spawn_randomness, spawn_randomness)
    ...
    def step(self, action):
        ...
        if self.current_stage == 0 and not self.stage_1_completed:
            ...
            if dist_to_truck_parking < self.stage_1_distance_threshold and abs(0.5 - angle_diff_) < self.stage_1_angle_threshold:
                self.current_stage = 1
                self.stage_1_completed = True
                print("\n*** STAGE 1 COMPLETE! Transitioning to Stage 2: Backing into Parking ***\n")
                # Recompute obs immediately so reward uses stage-2 target on transition step.
                obs = self._get_observation()
    ...
    def _calculate_reward(self, obs):
        ...
        distance = float(obs["distance_to_target"][0]) if not self.stage_1_completed else 0.0
        angle_diff = abs(float(obs["angle_difference"][0]))
        jackknife = float(obs["jackknife_angle"][0])
        
        # Proximity reward
        proximity_reward = 0.38 * np.cos(np.pi * distance ** (np.log(0.5) / np.log(0.75)))
        ...
        stage_bonus = 0.6 if self.stage_1_completed else 0.0
