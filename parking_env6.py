...
class ParkingLotEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.world.get_spectator().set_transform(carla.Transform(carla.Location(121.289558, 50.964546, 35.850121), carla.Rotation(-74.851120, -90.247307, -0.000359)))
        ...
        self.trailer_parking_point = find_obj(self.world, "trailer_parking")[0]
        self.cone_2 = find_obj(self.world, "SM_ConstructionCone2")[0]
        self.cone_6 = find_obj(self.world, "SM_ConstructionCone6")[0]
        # endregion
        ...
        self.stage_1_distance_threshold = 0.1  # meters
    ...
    def reset(self, seed=None, options=None):
        ...
        spawn_point = random.randrange(1,5,3)
        spawn_randomness = 2
        self.truck_spawn_point = find_obj(self.world, f"truck_spawn{spawn_point}", 0.1)[0]
        spawn_transform = self.truck_spawn_point.transform
        # Add randomness to spawn point location
        # spawn_transform.location.x += random.uniform(-spawn_randomness, spawn_randomness)
        spawn_transform.location.y += random.uniform(0, spawn_randomness)
        ...
        if not tick_ok: raise RuntimeError(f"World tick failed during reset: {tick_error}")
    ...
    def spawn_sensors(self):
        ...
        self.truck_radar_mounts = [
            ...
            (truck_bb.extent.x - 0.45, -truck_bb.extent.y + 0.04, 1, -45.0),  # Back Left Corner
            (truck_bb.extent.x - 0.45, truck_bb.extent.y - 0.04, 1, 45.0),   # Back Right Corner
    ...
    def _get_observation(self):
        ...
        trailer_parking_pose = get_actor_pose(self.trailer_parking_point)
        ...
        distance_to_target = np.linalg.norm(reference_point - trailer_parking_pose[:2])
        distance_to_target = min_max(distance_to_target, 0, 35)
        ...
        vec_target_point = trailer_parking_pose if self.stage_1_completed else truck_parking_pose
        vec_to_target = np.array([vec_target_point[0] - reference_point[0], vec_target_point[1] - reference_point[1]])
        ...
        # Position and distance calculations relative to target
        ...
        longitudinal_dist = min_max(longitudinal_dist, 0, 35)
        ...
        parallel_dist = min_max(parallel_dist, 0, 35)
    ...
    def step(self, action):
        ...
        if self.current_stage == 0 and not self.stage_1_completed:
            trailer_x, trailer_y, trailer_yaw = get_actor_pose(self.trailer)
            cone_2 = get_actor_pose(self.cone_2)
            cone_6 = get_actor_pose(self.cone_6)
            ...
            dist_to_cone2 = np.linalg.norm(reference_point - cone_2[:2])
            dist_to_cone6 = np.linalg.norm(reference_point - cone_6[:2])
            angle_diff_ = float(obs["angle_difference"][0])
                        
            # Check if trailer reached the positioning point
            if (dist_to_cone2 < 6.5 and
                dist_to_cone6 < 6.5 and
                abs(0.5 - angle_diff_) < self.stage_1_angle_threshold
            ):
        ...
        if self.current_stage == 1:
            ...
            parking_pose_check = get_actor_pose(self.trailer_parking_point)
        ...
        info["progress"] = {
            "stage_1_complete": int(self.stage_1_completed),
        ...
        hud_location = spectator_transform.location + forward * 5.0 + right * 3.4 + up * -1
        ...
        self.world.debug.draw_string(
            ...
            text,
            draw_shadow=True,
            color=carla.Color(r=20, g=40, b=15) if self.current_stage == 0 else carla.Color(r=255, g=165, b=0),
            life_time=0.05,
        )
        # endregion
    ...
    def _calculate_reward(self, obs):
        ...
        # Proximity reward
        proximity_reward = 0.9 * 0.6 * (1 - distance / 0.75)
        
        # Reward for alignment
        y_pre = (1 - distance) * np.cos(2 * math.pi * phi - math.pi)
        alignment_reward = 0.9 * max(0, 0.4 * y_pre) + min(0, 0.3 * y_pre)
        
        # === 3. Jackknife Penalty ===
        jackknife_penalty = self._calculate_jackknife_penalty(jackknife)

        # === 4. Stage Completion Bonus ===
        stage_bonus = 0.1 if self.stage_1_completed else 0.0
